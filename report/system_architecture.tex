...
\begin{figure*}[t]
    ...
    \begin{tikzpicture}[
        ...
        data/.style={rectangle, draw, fill=green!20, text width=2.2cm, align=center, minimum height=0.9cm, rounded corners=4pt, font=\footnotesize\bfseries, line width=1.2pt},
    ...
    % Layer 5: Outputs
    \node[output] (submission) at (-2.5, -3.5) {Kaggle\\Submission\\{\tiny predictions.csv}};
    \node[output] (models) at (0, -3.5) {Trained\\Models\\{\tiny Checkpoints}};
    \node[output] (viz) at (2.5, -3.5) {Visualizations\\{\tiny Confusion Matrix}\\{\tiny Learning Curves}};
    ...
    % Final outputs
    \draw[arrow] (grid) -- (submission);
    \draw[arrow] (grid) -- (models);
    \draw[arrow] (early) -- (submission);
    \draw[arrow] (early) -- (viz);
    ...
    % Title
    \node[font=\Large\bfseries] at (0, 7.8) {System Architecture Overview};
