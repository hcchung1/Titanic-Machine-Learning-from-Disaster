...
Figure~\ref{fig:confusion_matrices} presents confusion matrices for the top two models: Random Forest (RF features) and XGBoost (XGB features).

\begin{figure}[t]
    \centering
    \includegraphics[width=0.48\columnwidth]{figures/confusion_matrix_rf.png}
    \includegraphics[width=0.48\columnwidth]{figures/confusion_matrix_xgb.png}
    \caption{Confusion matrices for (left) Random Forest with RF features and (right) XGBoost with XGB features. Both models exhibit higher precision on the ``Did Not Survive'' class due to class imbalance (62\% majority).}
...
\begin{enumerate}
    ...
    \item \textbf{Random Forest with community-driven features is the most robust model:} Achieving 79.67\% validation accuracy with low variance across seeds.
